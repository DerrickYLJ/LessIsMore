...
def llama_tidal_attention_forward(
    ...
    lim_ratio=1,  # @artij: how to split the KV cache -> token_budget/lim_ratio amount of tokens to top-k, rest for window/sink tokens --> if 1 then all for top-k
    **kwargs,
):
    ...
    if self.layer_idx < sparse_layer_start or q_len == kv_seq_len:
        ...
        token_budget = min(
            last_dim_size, top_k
        )  # token budget controls sparsity level, limits number of tokens attended to, balances efficiency vs performance
        ...
        if (
        ...
        ) and top_k >= last_dim_size:  # if there is no need to do sparse attention
            self.pos_dict = torch.ones_like(attn_weights)
        elif self.layer_idx == sparse_layer_start or self.layer_idx == correction_layer:
            # @artij: keep attention sink, (token_budget // lim_ratio) for top-k, token_budget - (token_budget // lim_ratio) for the window attention/sink tokens
            # @artij: priority is to keep the most recent tokens over top-k

            middle_budget = int(token_budget * (1 - lim_ratio))  # top-k
            ...
            recent_start = (
                last_dim_size - most_recent_amount
            )  # total seq len - most_recent_amount
            ...
            top_k_mask = torch.zeros_like(attn_weights).scatter_(
                -1, top_k_indices, 1.0
            )  # puts a 1.0 at the indices in tok_k --> mask out the non-top k tokens
...
def llama_tidal_forward_yarn(
    ...
    if (
        self.layer_id == self.sparse_layer_start
        or self.layer_id == self.correction_layer
    ):
        # extract top_k mask
...
def enable_llama_tidal_attention(
    ...
    def wrap_forward(
        module,
    ):  # @artij: creates a new forward function that uses tidal attention
    ...
    for name, module in reversed(model._modules.items()):
        ...
        print(f"model: {base_class.__module__ }", flush=True)

        if (
            base_class.__module__ == "src.models.llama_tidaldecoding"
            and base_class.__name__ == "LlamaAttention"
        ):
            wrap_forward(module)
