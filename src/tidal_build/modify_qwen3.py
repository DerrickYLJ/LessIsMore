    ...
    **kwargs: Unpack[FlashAttentionKwargs],
    ...
    if self.layer_idx < sparse_layer_start or q_len == kv_seq_len:
        ...
        if self.layer_idx == sparse_layer_start or self.layer_idx == correction_layer:
            ...
            # ### start: randomize attn head for kv group
            # num_kv_groups = 8
            # heads_per_group = 4
            ...
            #     selected_head = torch.randint(start_head, end_head, (1,)).item()
            #     selected_indices = self.pos_index[
            #         :, selected_head : selected_head + 1, :, :
            #     ]
            #     self.pos_index[:, start_head:end_head, :, :] = selected_indices
            # ### end: randomize attn head for kv group
        else:
...
def enable_qwen_tidal_attention(
    ...
    """
    Enable TidalDecode sparse attention for Qwen3 model
        ...
        def new_tidal_forward(
            ...
            **kwargs: Unpack[FlashAttentionKwargs],
            ...
            return qwen_tidal_attention_forward(
                ...
                attention_sink=0,  # Not used in Qwen3
                **kwargs,
            )
    ...
    for name, module in reversed(model._modules.items()):
        ...
        if module.__class__.__name__ == "Qwen3Attention":
            print(f"Applying TidalDecode to layer {module.layer_idx}: {name}")
            print(f"TidalDecode enabled for Qwen3 model with:")
            print(f"  - top_k: {top_k}")
